...
import { addExpense } from "@/validations/expense";
...
export default function AddExpenseBtn() {
  ...
  const {
    ...
    resolver: yupResolver(addExpense),
  });
  ...
  const mutation = useMutation({
    mutationFn: async (data: addExpenseFormData) => {
      ...
      const makeRequest = async (token: string) => {
        return await fetch("http://localhost:4002/api/v1/expenses", {
          method: "POST",
          headers: {
    ...
    onError: () => {
      toast.error("خطا در افزودن هزینه");
    },
  });

  const addExpenseHandle = async (data: addExpenseFormData) => {
      ...
      {openModal ? (
        <Modal onClose={modalToggleHandle}>
          <>
            ...
            <form
              onSubmit={handleSubmit(addExpenseHandle)}
