...
import { addCheck } from "@/validations/check";
  ...
  const mutation = useMutation({
    mutationFn: async (data: addItemFormData) => {
      ...
      const makeRequest = async (token: string) => {
        return await fetch("http://localhost:4002/api/v1/checks", {
          method: "POST",
          headers: {
      ...
      if (result.statusCode !== 201) throw new Error("Failed to add check");
          ...
          <>
            ...
            >
              ...
              <div className="w-full lg:max-w-5/12">
                <div className="flex items-center gap-2">
                  <div className="w-10 h-10 xs:w-12 xs:h-12 flex justify-center items-center rounded-full bg-[var(--color-secondary)]">
                    <IconStatus size="w-6 h-6 xs:w-7 xs:h-7" color="#fff" />
                  </div>
                  <Controller
                    name="status"
                    control={control}
              ...
              <div className="w-full lg:max-w-5/12">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    placeholder=" سریال  را وارد کنید (اختیاری)"
                  />
                </div>
